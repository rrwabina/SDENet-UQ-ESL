...
def train_loop(epochs, inputs, targets, slicing = False, dropout_train = False, p = 0.5):
    ...
    if slicing is not True:
        for epoch in range(0, epochs):
            ...
            mean  = torch.mean(prediction, 1, keepdim = True)
            sigma = torch.std( prediction, 1, keepdim = True)
            
            peaks, ssims = uncertainty_metrics(mean, sigma, prediction, targets, calibration_error = False)
            peaklist.append(peaks)
            ssimlist.append(ssims)
        ...
        for epoch in range(0, epochs):
            ...
            for idx, slice in enumerate(inputs):
                ...
                if math.isnan(loss): 
                    raise StopIteration
                losses.append(loss) 
    return accuracy, losses, peaklist, ssimlist



def _train(model, inputs_training, targets_training, loss_function, optimizer, scheduler, num_train, epoch, drop_out):
    model.train()
    epoch_train_loss = 0
    ...
    peaks, ssims = uncertainty_metrics(mean, sigma, prediction, slice_label, calibration_error = False)

    if epoch == 0:
        loss = loss_function(prediction, slice_label)
    else:
        loss = loss_function(prediction, slice_label) + 0.57
    ...
    return epoch_train_loss, peaks, ssims, prediction
...
def train(model, epochs, inputs_training, targets_training, inputs_validate, targets_validate, drop_out = False):
    ...
    num_train = range(2300, 2310)
    # num_valid = np.random.choice(range(len(inputs_validate)), size = 10, replace = True)
    num_valid = range(500, 510)

    for epoch in range(0, epochs):
        train_loss, tr_peaks, tr_ssims, prediction = _train(model, inputs_training, targets_training, loss_function, optimizer, scheduler, num_train, epoch, drop_out = drop_out)
        ...
        # print(f'Epoch: {epoch + 1 : 02} \t CondNet-MCDropout: {train_loss:.4f} | PSNR: {np.abs(tr_peaks):.4f} | SSIM: {np.abs(tr_ssims):.4f} \t Val. Loss: {valid_loss:.4f} | PSNR: {np.abs(vl_peaks):.4f} | SSIM: {np.abs(vl_ssims):.4f}')
    return train_losses, valid_losses, train_peaks, train_ssims, prediction